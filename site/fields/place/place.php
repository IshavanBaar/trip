<?php class PlaceField extends TextField {
  ...
  public function __construct() {
    // Default location (Sumatra)
    $jsonLat = -6.226656;
    $jsonLng = 106.4285054;  
    $zoom = 3;
      
    // Get current location
    $jsonString = @file_get_contents("http://ip-api.com/json");
    if($jsonString !== FALSE) {
        $jsonData = json_decode($jsonString, true);
        if ($jsonData['status'] !== "fail") {
            $jsonLat = $jsonData['lat'];
            $jsonLng = $jsonData['lon'];
            $zoom = 9;
        } 
    }
    ...
    $this->map_settings = array(
      'lat' => c::get('place.defaults.lat', $jsonLat),
      'lng' => c::get('place.defaults.lng', $jsonLng),
      'zoom' => c::get('place.defaults.zoom', $zoom)
    );
  }
